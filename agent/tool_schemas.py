...
TRIGGER_RECOMMEND_ASTROLOGER = {
    ...
    "description": (
        ...
        "The result's next_available_at is an estimated return time — "
        "when it's set, say that directly ('back in about 40 minutes') "
        "rather than a vague 'I'll let you know when they're back', AND "
        "in that SAME reply push connecting with someone else who's "
        "available right now instead of waiting — call this tool again "
        "WITHOUT astrologer_id (general/best-match) to surface that "
        "option; don't just state the wait and stop there. You can still "
        "offer notify_me_subscribe on top of both. When next_available_at "
        "is null, there's no known return time at all — lead with the "
        "connect-with-someone-else push, notify_me_subscribe's "
        "push-notification framing is the fallback. "
...
NOTIFY_ME_SUBSCRIBE = {
    ...
    "description": (
        ...
        "now — ALWAYS also call trigger_recommend_astrologer (no "
        "astrologer_id, so it picks someone available right now) in the "
        "SAME reply for a live alternate, don't just say 'keep checking "
        "the app'. The result's next_available_at is an estimated return "
